...
interface IngestCliOptions {
  ...
  /** Enable multimodal LLM API for image captioning in documents */
...
Options:
  ...
  --multimodal               Use configured VLM API (VLM_API_* env vars) to describe images in documents and interleave descriptions with text
  ...
  while (i < args.length) {
    ...
    switch (arg) {
      ...
      case '--visual':
        ...
        break
      case '--multimodal':
        // Boolean toggle: enable multimodal LLM image captioning
...
export type IngestSingleFileOptions =
  | { visual?: false | undefined; multimodal?: false | undefined }
  | {
      visual: true
...
export async function ingestSingleFile(
  ...
  if (options?.multimodal === true && 'vlmConfig' in options) {
    ...
    const vectorChunks: VectorChunk[] = result.chunks.map((chunk, index) => {
...
export async function runIngest(args: string[], globalOptions: GlobalOptions = {}): Promise<void> {
  ...
  try {
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        // Forward visual / multimodal VLM env-resolved options into the per-file ingestor.
        let ingestOptions: IngestSingleFileOptions;
        if (options.multimodal) {
          const vlmApiBaseUrl = process.env['VLM_API_BASE_URL'];
          const vlmApiKey = process.env['VLM_API_KEY'];
          const vlmApiModel = process.env['VLM_API_MODEL'];
          if (!vlmApiBaseUrl || !vlmApiKey || !vlmApiModel) {
            console.error(
              `${label} ${filePath} ... FAILED: VLM_API_BASE_URL, VLM_API_KEY, and VLM_API_MODEL must be set for --multimodal`,
            );
            summary.failed++;
            continue;
          }
          ingestOptions = {
            multimodal: true,
            vlmConfig: { apiBaseUrl: vlmApiBaseUrl, apiKey: vlmApiKey, apiModel: vlmApiModel },
          };
