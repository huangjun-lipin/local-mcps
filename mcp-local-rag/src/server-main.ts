 ...
 */
export function parseGroupingMode(
  value: string | undefined,
): ParseResult<GroupingMode> {
 ...
 */
export function parseMaxDistance(
  value: string | undefined,
): ParseResult<number> {
  if (!value) return { value: undefined };
 ...
 */
export function parseHybridWeight(
  value: string | undefined,
): ParseResult<number> {
  if (!value) return { value: undefined };
  const parsed = Number.parseFloat(value);
 ...
 */
export function parseChunkMinLength(
  value: string | undefined,
): ParseResult<number> {
  if (!value) return { value: undefined };
  const parsed = Number.parseInt(value, 10);
...
export async function startServer(): Promise<void> {
  try {
    ...
    const config: ConstructorParameters<typeof RAGServer>[0] = {
      ...
      embeddingProvider:
        (process.env['EMBEDDING_PROVIDER'] as 'local' | 'api' | undefined) ||
        'local',
      embeddingApiBaseUrl: process.env['EMBEDDING_API_BASE_URL'],
      embeddingApiKey: process.env['EMBEDDING_API_KEY'],
      embeddingApiModel: process.env['EMBEDDING_API_MODEL'],
      ...
      device,
      vlmApiBaseUrl: process.env['VLM_API_BASE_URL'],
      vlmApiKey: process.env['VLM_API_KEY'],
      vlmApiModel: process.env['VLM_API_MODEL'],
    };
