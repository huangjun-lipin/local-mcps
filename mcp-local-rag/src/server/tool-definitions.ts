...
export const toolDefinitions: Tool[] = [
  ...
  },
  {
    name: 'ingest_file',
    description:
      'Ingest a document file (PDF, DOCX, TXT, MD, XLSX, PPTX, PNG, JPG, etc.) into the vector database for semantic search. File path must be an absolute path. Supports re-ingestion to update existing documents. Use multimodal=true to have images in documents described by a configured VLM API before chunking.',
    inputSchema: {
      type: 'object',
      properties: {
        filePath: {
        ...
        visual: {
          type: 'boolean',
          description:
            'If true and the file is a PDF, run VLM captioning on figure-heavy pages using the local VLM pipeline. No effect on non-PDF files. Takes lower precedence than multimodal.',
        },
        visualQuality: {
          type: 'string',
          enum: ['fast', 'quality'],
          default: 'fast',
          description:
            'VLM profile to use when visual is true. "fast" (default) is the lightweight SmolVLM-256M; "quality" is Qwen2.5-VL-3B-Instruct-ONNX with higher fidelity on figures with in-image text. Ignored when visual is false.',
        },
        multimodal: {
          type: 'boolean',
          description:
            'If true, use the configured multimodal LLM API (VLM_API_BASE_URL/VLM_API_KEY/VLM_API_MODEL) to describe images within the document (works with DOCX, PDF, XLSX, PPTX, and image files). Image descriptions are interleaved with text in the original document order before chunking and embedding. Requires server-side VLM API configuration.',
        },
      },
      required: ['filePath'],
    },
