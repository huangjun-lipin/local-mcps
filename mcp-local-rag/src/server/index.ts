...
import { multimodalIngest } from '../ingest/multimodal.js';
import { prepareVisualPdfChunks } from '../ingest/visual.js';
import { MultimodalError } from '../multimodal/types.js';
...
export class RAGServer {
  ...
  constructor(config: RAGServerConfig) {
    ...
    const embedderConfig: ConstructorParameters<typeof Embedder>[0] = {
      ...
      apiBaseUrl: config.embeddingApiBaseUrl,
      apiKey: config.embeddingApiKey,
      apiModel: config.embeddingApiModel,
    };
  ...
  async handleIngestFile(
    ...
    // Runtime validation + normalization of `visualQuality`.
    ...
    // Runtime validation for `multimodal` flag
    const multimodalArg: unknown = (args as { multimodal?: unknown }).multimodal;
    const useMultimodal = multimodalArg === true;
    if (multimodalArg !== undefined && typeof multimodalArg !== 'boolean') {
      throw new McpError(ErrorCode.InvalidParams, "'multimodal' must be a boolean if provided");
    }
    ...
    try {
      ...
      if (isRawDataPath(args.filePath)) {
        ...
        // Multimodal path: extract images + caption via VLM API + interleave
        if (!this.config.vlmApiBaseUrl || !this.config.vlmApiKey || !this.config.vlmApiModel) {
          throw new McpError(
            ErrorCode.InvalidParams,
            'Multimodal ingest requires VLM_API_BASE_URL, VLM_API_KEY, and VLM_API_MODEL to be configured on the server.',
          );
        }
        const vlmConfig = {
          apiBaseUrl: this.config.vlmApiBaseUrl,
          apiKey: this.config.vlmApiKey,
          apiModel: this.config.vlmApiModel,
        };
        try {
          const result = await multimodalIngest(
            args.filePath,
            vlmConfig,
            this.chunker,
            this.embedder,
      ...
      try {
        ...
        const existingFile = existingFiles.find(
          (file) => file.filePath === args.filePath,
        );
        if (existingFile && existingFile.chunkCount > 0) {
          // Backup existing data (retrieve via search)
          const queryVector = embeddings[0] || [];
          if (queryVector.length > 0) {
            const allChunks = await this.vectorStore.search(
              queryVector,
              undefined,
              20,
            ); // Retrieve max 20 items
            backup = allChunks
          ...
          console.error(
            `Backup created: ${backup?.length || 0} chunks for ${args.filePath}`,
          );
        }
      } catch (error) {
      ...
      try {
        ...
        console.error(
          `Inserted ${vectorChunks.length} chunks for: ${args.filePath}`,
        );
        ...
        if (backup && backup.length > 0) {
          ...
          try {
            ...
            await this.vectorStore.optimize();
            console.error(
              `Rollback completed: ${backup.length} chunks restored`,
            );
          } catch (rollbackError) {
  ...
  async handleIngestData(
    ...
    try {
      ...
      if (args.metadata.format === 'html') {
        ...
        console.error(
          `Converted HTML to Markdown: ${markdown.length} characters`,
        );
      ...
      console.error(
        `Saved raw data: ${args.metadata.source} -> ${rawDataPath}`,
      );
    ...
    try {
      ...
      const baseDirFiles = entries
        .filter(
          (e) =>
            e.isFile() &&
            SUPPORTED_EXTENSIONS.has(extname(e.name).toLowerCase()),
        )
        .map((e) => {
        ...
        .filter(
          (filePath) =>
            !this.excludePaths.some((ep) => filePath.startsWith(ep)),
        )
        .sort();
    ...
    try {
      ...
      if (isRawDataPath(targetPath)) {
        try {
          ...
          console.warn(
            `Could not delete raw-data file (may not exist): ${targetPath}`,
          );
        }
        try {
          await unlink(generateMetaJsonPath(targetPath));
          console.error(
            `Deleted meta.json: ${generateMetaJsonPath(targetPath)}`,
          );
        } catch {
  ...
  async handleReadChunkNeighbors(
    ...
    try {
      ...
      if (after > 50) {
        throw new McpError(
          ErrorCode.InvalidParams,
          `after must be between 0 and 50 (got ${after})`,
        );
      }

      // Resolve target filePath (XOR: filePath vs source)
      let targetPath: string;
      if (args.filePath && args.source) {
        throw new McpError(
          ErrorCode.InvalidParams,
          'Provide exactly one of filePath or source, not both.',
        );
      }
      if (args.filePath) {
        targetPath = args.filePath;
      ...
      // Query neighbors from vector store
      const neighbors = await this.vectorStore.readNeighbors(
        targetPath,
        args.chunkIndex,
        before,
        after,
      ...
      // Map to MCP tool output items
      const items: ReadChunkNeighborsResultItem[] = neighbors.map((n) => {
        const item: ReadChunkNeighborsResultItem = {
          filePath: n.filePath,
          chunkIndex: n.chunkIndex,
          text: n.text,
          isTarget: n.chunkIndex === args.chunkIndex,
          fileTitle: n.fileTitle ?? null,
        };
        if (isRawDataPath(n.filePath)) {
          const source = extractSourceFromPath(n.filePath);
          if (source) {
            item.source = source;
          }
        }
