...
export interface RAGServerConfig {
  ...
  /** Multimodal LLM (VLM) API base URL for image captioning */
  vlmApiBaseUrl?: string;
  /** Multimodal LLM (VLM) API key */
  vlmApiKey?: string;
  /** Multimodal LLM (VLM) model name (e.g. gpt-4o, claude-3-opus, qwen-vl-max) */
  vlmApiModel?: string;
}
...
export interface IngestFileInput {
  ...
  /**
   * When true, use the configured multimodal LLM API to describe images
   * within the document (DOCX/PDF/XLSX/PPTX/image files) and interleave
   * descriptions with text in original document order before chunking.
   * Requires VLM_API_BASE_URL, VLM_API_KEY, and VLM_API_MODEL to be set.
   * For PDF files, this takes precedence over the local `visual` path.
   */
  multimodal?: boolean;
}
