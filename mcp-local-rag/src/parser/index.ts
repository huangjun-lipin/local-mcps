...
export class DocumentParser {
  ...
  /**
   * XLSX parsing (text-only: shared strings + cell values).
   * For the full multimodal path with image extraction, use src/multimodal/.
   *
   * @param filePath - XLSX file path
   * @returns ParseResult with content and extracted title
   * @throws FileOperationError - File read failed
   */
  private async parseXlsx(filePath: string): Promise<ParseResult> {
    try {
      const buffer = await readFile(filePath);
      const { readZip, parseSharedStrings, parseSheetText } = await import('./xlsx-parser.js');
      const zipFiles = await readZip(buffer);
      const sharedStrings = parseSharedStrings(zipFiles);
      const text = parseSheetText(zipFiles, sharedStrings);
      const fileName = basename(filePath);
      const title = fileName;
      console.error(`Parsed XLSX: ${filePath} (${text.length} characters)`);
  ...
  /**
   * PPTX parsing (text-only: slide text content).
   * For the full multimodal path with image extraction, use src/multimodal/.
   *
  ...
  private async parsePptx(filePath: string): Promise<ParseResult> {
    try {
      const buffer = await readFile(filePath);
      const { readZip, parseSlideText } = await import('./pptx-parser.js');
      const zipFiles = await readZip(buffer);
      const text = parseSlideText(zipFiles);
      const fileName = basename(filePath);
      const title = fileName;
      console.error(`Parsed PPTX: ${filePath} (${text.length} characters)`);
      return { content: text, title };
    } catch (error) {
  ...
  /**
   * Image file parsing — returns the file name as placeholder text.
   * Real image content requires multimodal LLM captioning via the multimodal path.
   *
   * @param filePath - Image file path
   * @returns ParseResult with placeholder content
   * @throws FileOperationError - File read failed
   */
  private async parseImage(filePath: string): Promise<ParseResult> {
    try {
      ...
      console.error(`Parsed image: ${filePath} (${ext})`);
      return { content: text, title: fileName };
    } catch (error) {
      throw new FileOperationError(`Failed to parse image: ${filePath}`, error as Error);
    }
  }
}
