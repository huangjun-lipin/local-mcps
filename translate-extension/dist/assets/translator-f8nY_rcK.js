import{g as l,D as h}from"./languages-s6Qk9-53.js";const y="https://api.openai.com/v1/chat/completions",L="gpt-4o-mini";async function d(){const t=await chrome.storage.sync.get(["apiUrl","apiKey","model","targetLang"]);return{apiUrl:t.apiUrl||y,apiKey:t.apiKey||"",model:t.model||L,targetLang:t.targetLang||h}}async function u(t){var s,i,c,g;const e=await d();if(!e.apiKey)throw new Error("请先在扩展设置中配置 API Key");if(!t||t.trim().length===0)return"";const n=l(e.targetLang),o=n?`${n.zhName}（${n.nativeName}）`:e.targetLang,m=`你是一个专业的翻译助手。你的任务是将用户提供的任何文字翻译成${o}。
重要规则：
